        ...
        <div class="card-body">
            <form action="{{ isset($property) ? route('properties.update', $property->id) : route('properties.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="units" name="units" value="{{ old('units', $property->units ?? '') }}" required>
                </div>
                
                <div class="mb-3">
                    <label for="rental_cost" class="form-label">Rental Cost</label>
                    <input type="text" class="form-control" id="rental_cost" name="rental_cost" value="{{ old('rental_cost', $property->rental_cost ?? '') }}" required>
                </div>

                <button type="submit" class="btn btn-info">{{ isset($property) ? 'Update Property' : 'Add Property' }}</button>
            </form>
        </div>
    </div>
</div>
